...
func Create(w http.ResponseWriter, r *http.Request) {
	...
	err = s3.UploadFiles(os.Getenv("AWS_BIN_FILE_DEST_DIR")+video.FileName+".bin", fileContent)
	if err != nil {
	...
	// Update DB
	video.S3Location = os.Getenv("AWS_BIN_FILE_DEST_DIR") + video.FileName + ".bin"
	video.Status = domain.StatusDone
	err = application.UpdateVideo(&video)
	if err != nil {
		tools.FormatResponseBody(w, http.StatusInternalServerError, "Cannot update video status, err : "+err.Error())
		return
	}
